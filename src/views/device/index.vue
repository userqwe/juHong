      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <!--<el-button @click="handleClick(scope.row, 1)" class="margin10" type="success" size="small" plain>发布广告</el-button>-->
            <!--<el-button @click="handleClick(scope.row, 2)" class="margin10" type="primary" size="small"  plain>播放列表</el-button>-->
            <!--<el-button @click="handleClick(scope.row, 3)" class="margin10" size="small" plain>远程控制</el-button>-->
            <el-button @click="handleClick(scope.row, 4)" class="margin10" type="info" size="small" plain>编辑设备</el-button>
            <!--<el-button @click="handleClick(scope.row, 5)" class="margin10" type="warning" size="small" plain>删除设备</el-button>-->
            <el-button @click="handleClick(scope.row, 6)" class="margin10" type="danger" size="small" plain>移除设备组</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  import { getDeviceList, removeDeviceFromGroup, getDevGroupList } from '@/api/device'
    ...
    methods: {
      ...
      fetchData(page, size) {
        ...
        getDevGroupList(0, 10).then(res => {
          this.devGroupList = res.data.list
          getDeviceList(page, size).then(response => {
            let tempData = []
            this.deviceList = response.data.list
            this.deviceList.map(item => {
              tempData = this.devGroupList.filter(group => group.id == item.group_id)[0]
              if (tempData) {
                item.group_name = tempData.group_name
              } else {
                item.group_name = '未设定'
              }
              item.status_name = item.open == 1 ? '开启' : '关闭'
            })
      ...
      handleClick(data, type) {
        if (type === 4) {
          this.$router.push({ path: '/device/add', query: { dev_id: data.device_id }})
        }
        if (type === 6) {
          this.$confirm('确定将该设备从设备组移除吗?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning',
            center: true
          }).then(() => {
            removeDeviceFromGroup(data.device_id, data.group_id).then(res => {
              if (res.code === 200) {
                Message({
                  message: '设备移除成功',
                  type: 'success',
