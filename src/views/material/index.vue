  ...
  <div class="app-container">
    <el-form :inline="true" :model="pageData" class="demo-form-inline">
      ...
      <div class="func-btn-box">
        <el-button type="primary" @click="addClassifyType(true)">新增分类</el-button>
        <el-button type="success" @click="addClassifyType(false)">编辑当前分类</el-button>
      </div>
    </el-form>
    ...
    <div class="table">
      <el-table
        ...
        <el-table-column
          label="操作" width="200">
          <template slot-scope="scope">
            <el-button @click="eidtMaterial(scope.row)" type="warning" size="small" plain>编辑</el-button>
            <el-button @click="delMaterial(scope.row.id)" type="danger" size="small" plain>删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
  import { getMaterialList, editMaterial, delMaterial, getMaterialTypeList, addMaterialType, editMaterialType } from '@/api/material'
  ...
  export default {
    ...
    methods: {
      ...
      // 删除素材
      delMaterial(id) {
        this.$confirm('确认删除该素材吗？')
          .then(_ => {
            delMaterial(id, 0).then(response => {
              if (response.code === 200) {
                Message({
                  message: '素材已删除',
                  type: 'success',
